...
import "./blocks/javascript/chameleon";
...
import { registerBuiltinFields } from "./fields/builtins";
import { registerExtensionFields } from "./fields/extensions";
...
function globalSetup() {
  ...
  // Register blocks.
  Object.values(objects.BLOCKS).forEach(b => b.defineBlock());
  Object.values(objects.BLOCKS).forEach(b => b.defineToolboxEntry());

  Object.values(random.BLOCKS).forEach(b => b.defineBlock());
  Object.values(random.BLOCKS).forEach(b => b.defineToolboxEntry());

  Object.values(text.BLOCKS).forEach(b => b.defineBlock());
  Object.values(text.BLOCKS).forEach(b => b.defineToolboxEntry());
}

const workspace = Workspace.create(BLOCKLY_DIV);
...
const runCode = async () => {
  ...
  try {
    ...
    code = (e as object).toString().replace(/^/gm, "// ");
  }
