...
export function toJson(translation: unknown) {
...
export function colourFromString(str: string): string {
...
const MESSAGE_REGEX = verboseRegExp`
    ...
    (?: [$][{] ( [^\}]+ ) [}] )   // Capture interpolation pattern
  | (   \n )                      // Capture newlines
  | (   [^\n$]+ )                 // Capture any other text
`;
